# Data Description — EtseeMate Listing DB
...
|No.| Bảng | Mô tả | Status |
| --- |---|---|---|
|1 | `listings` | Master list các listing đang theo dõi (theo dõi unique EtseeMate listing_id từ `listing_report` và `manul_keyword_report`)| Data clean|
|2 | `listings_int_ext` | Báo cáo period range mới nhất của mỗi listing, bao gồm EtseeMate indicator (views/clicks/orders/revenue/spend/roas + cpc/cpp nội suy) và scenario (roas_band, cr_level, ctr_level + scenarios_rules) | Materialized — populate bởi `reporting_etl.rebuild_reporting()` |
|3 | `keywords` | Báo cáo keyword của import_time mới nhất mỗi listing, bao gồm EtseeMate indicator + cpc/cpp nội suy | Materialized — populate bởi `reporting_etl.rebuild_reporting()` |
|4 | `listings_int_hist` | Lịch sử daily (period dạng `YYYY-MM-DD`) của mỗi listing, bao gồm EtseeMate indicator + cpc/cpp nội suy | Materialized — populate bởi `reporting_etl.rebuild_reporting()` |
...
* EtseeMate indicators: views, clicks, orders, revenue, spend, roas, và 2 trường nội suy cpc=spend/clicks và cpp= spend/orders
...
- `POST /api/v1/EtseeMate/confirm` (auto rebuild ngay sau khi import manual batch xong)
...
|5| `importer` | Định danh source: `EtseeMate_bot`, `EtseeMate_json`, `EtseeMate_crawler`, `manual`… |
...
Hiệu suất ads của từng listing theo từng period hoặc ngày đơn lẻ (daily breakdown). Với `listing_report` là kết quả của luồng crawl EtseeMate listing tự động (không có trong source code này), `manual_listing_report` là kết quả của  luồng **Manual Import**
...
| `importer` | `VARCHAR(64)` | `String(64)` | YES | Source: `EtseeMate_json`, `EtseeMate_crawler`… |
...
| `import_time` | `TIMESTAMPTZ` | `DateTime(tz=True)` | YES | Thời điểm import |
| `importer` | `VARCHAR(64)` | `String(64)` | YES | Source: `EtseeMate_bot`, `EtseeMate_crawler`… |
...
Dữ liệu thị trường mở rộng cho tập EtseeMate listing, crawl từ Etsy search — dùng cho Research Hub.
...
| `importer` | `VARCHAR(64)` | `String(64)` | YES | Source import (EtseeMate_bot, manual…) |

---

### `listings_int_ext`

Materialized overview — 1 row per `(listing_id, period)` cho period range mới nhất. **EtseeMate indicators only** — các trường external (price, rating, badge, …) KHÔNG materialize ở đây, được join từ `etsy_market_db.market_listing` tại runtime trong `get_dashboard_listings()`.
