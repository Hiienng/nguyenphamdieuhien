...
# Design System Inspired by Claude (Anthropic) - v2
...
- **Ivory** (`#faf9f5`): The lightest surface — used for cards and elevated containers on the Parchment background.
- **Dark Surface** (`#30302e`): Dark-theme containers, nav borders, and elevated dark elements — warm charcoal.
...
|------|------|------|--------|-------------|-------|
| Display / Hero | Anthropic Serif | 64px (4rem) | 500 | 1.10 | Desktop primary headline |
| Section Heading | Anthropic Serif | 52px (3.25rem) | 500 | 1.20 | Feature section anchors |
| Sub-heading | Anthropic Serif | 32px (2rem) | 500 | 1.10 | Card titles, feature names |
| Body Standard | Anthropic Sans | 16px (1rem) | 400 | 1.60 | Standard body text |

## 4. Component Stylings

### Buttons
- **Brand Terracotta**: Background: `#c96442`, Text: `#faf9f5`, Radius: `8-12px`.
- **Warm Sand (Secondary)**: Background: `#e8e6dc`, Text: `#4d4c48`, Radius: `8px`.

### Cards & Containers
- **Border**: Thin solid Border Cream (`1px solid #f0eee6`) or Border Warm (`1px solid #e8e6dc`).
- **Shadow**: Ring-based (`0px 0px 0px 1px`) pattern for subtle depth.

## 5. Layout Principles
- **Editorial pacing**: Generous whitespace between sections (80-120px).
- **Alternation**: Light and dark sections to create a reading rhythm.

## 6. Depth & Elevation
- **Ring Shadow Philosophy**: Use `0px 0px 0px 1px` warm-toned rings instead of heavy drop shadows.

## 7. Do's and Don'ts
- **Do**: Keep all neutrals warm-toned. Use single-weight (500) serifs for headlines.
- **Don't**: Use cool blue-grays. Use sharp corners (<6px).
...
## 9. Agent Prompt Guide
(Specific instructions for AI models to generate UI following this system)

## 10. Mobile-First Patterns & UX

Trên thiết bị di động, Claude chuyển dịch từ một "trang báo editorial" sang một "cuốn sổ tay cao cấp". Mục tiêu là duy trì sự ấm áp của chất liệu giấy (`Parchment`) trong khi vẫn đảm bảo tính tiện dụng trên màn hình cảm ứng.

### 10.1 Layout & Sheet System
- **Edge-to-Edge Canvas**: Trên mobile, các phần highlight màu `Near Black (#141413)` sẽ tràn viền hoàn toàn để tạo nhịp ngắt trang rõ rệt.
- **Bottom Sheets (Sự ưu tiên hàng đầu)**: Thay vì sử dụng Modal hay Dropdown ở giữa màn hình, Claude sử dụng Bottom Sheets trượt từ dưới lên cho các tác vụ chọn Model, Menu tùy chỉnh hoặc Upload.
    - **Radius**: Bo tròn góc trên `24px`, phẳng ở góc dưới.
    - **Handlebar**: Một thanh ngang nhỏ màu `Stone Gray (#87867f)` ở trên cùng để gợi ý cử chỉ vuốt xuống để đóng.
- **Safe Area Padding**: Luôn duy trì khoảng trống tối thiểu `16px` ở hai bên sườn (gutter) để văn bản không bị dính vào mép màn hình.

### 10.2 Mobile Typography Scale
| Role | Mobile Size | Line Height | Notes |
|------|-------------|-------------|-------|
| Hero Headline | 32px - 36px | 1.15 | Giảm từ 64px để tránh ngắt dòng quá nhiều |
| Section Header | 28px | 1.20 | Dùng cho các tiêu đề tính năng |
| Body Standard | 16px | 1.55 | Size tối thiểu để tránh iOS tự động auto-zoom khi focus |
| UI/Button Text | 15px | 1.00 | Tối ưu cho các nút bấm nhỏ hơn |

### 10.3 Mobile Input & Keyboard
- **Sticky Input Bar**: Khung soạn thảo tin nhắn dính chặt vào đáy màn hình hoặc đỉnh bàn phím. 
    - **Background**: `Ivory (#faf9f5)` với một lớp shadow ngược (ring shadow) lên phía trên để tách biệt với dòng chat.
- **Input Scaling**: Khi người dùng bắt đầu gõ, khung input có thể mở rộng tối đa 5 dòng trước khi kích hoạt thanh cuộn nội bộ.
- **Touch Targets**: Tất cả các icon (đính kèm, gửi, tùy chọn) phải nằm trong vùng chạm tối thiểu `44x44px`.

### 10.4 Gestures & Transitions (Cử chỉ & Chuyển cảnh)
- **Swipe to Sidebar**: Vuốt từ mép trái màn hình để mở danh sách hội thoại. 
- **Soft Transitions**: Sử dụng hiệu ứng **Fade & Slight Scale** (mờ dần và phóng to nhẹ) để tạo cảm giác nhẹ nhàng như lật một trang giấy mỏng.
- **Haptic Feedback**: Rung phản hồi cực nhẹ (Light impact) khi nhấn giữ tin nhắn hoặc khi Claude bắt đầu phản hồi.

### 10.5 Image & Media Mobile
- **Full-width Media**: Hình ảnh minh họa hoặc screenshot sản phẩm sẽ tự động tràn sát 2 mép màn hình (bleed-out).
- **Radius Adaptation**: Bo góc của ảnh giảm xuống còn `16px` (thay vì 32px trên desktop).

### 10.6 Mobile Navigation (The "Hamburger")
- **Icon**: Sử dụng 2 vạch ngang (thay vì 3) với độ dày trung bình để đồng bộ với `Anthropic Sans`.
- **Full-screen Overlay**: Khi mở menu, một lớp phủ màu `Parchment (#f5f4ed)` sẽ che toàn bộ màn hình, menu item trình bày như một mục lục sách font `Anthropic Serif`