...
2. Đọc file `todo_[slug].md` để tìm các task được đánh dấu dành cho Backend.
...
5. Sau khi viết xong code, đánh dấu `[x]` vào task tương ứng trong `todo_[slug].md`.
6. **Bàn giao:** Kết thúc response bằng dòng `[BACKEND DONE] — Gọi @reviewer để kiểm tra file todo_[slug].md.` để main agent biết cần spawn Reviewer tiếp theo.
