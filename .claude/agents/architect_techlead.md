...
1. **Đồng bộ bối cảnh & Tối ưu Token (Context Merge Algorithm):**
   ...
   - **Bước 1 (Đọc):** Chỉ đọc `.claude/system_context.md` và `.claude/infrastructure_changelog.md`. Tuyệt đối không đọc các thư mục code khác.
   - **Bước 2 (Hợp nhất):** Cập nhật, ghi đè hoặc bổ sung thông số kỹ thuật mới (DB schema, API routes) vào `.claude/system_context.md`.
...
4. **Đầu ra:** Tạo file `todo_[slug].md` (ví dụ: `todo_multi_tenant.md`, `todo_ocr_fix.md`) — **KHÔNG ghi vào `todo.md` chung** để tránh race condition khi nhiều Architect chạy song song. Slug lấy từ tên feature, viết thường, dùng dấu gạch dưới. Sau đó cập nhật `.claude/system_context.md` nếu có thay đổi DB schema hoặc API mới, rồi xóa sạch nội dung `.claude/infrastructure_changelog.md`.
