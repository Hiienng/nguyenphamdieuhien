  ...
  btnAddDb.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!response || !response.ok) {
        throw new Error(response && response.error ? response.error : "Insert failed");
      }
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (response && response.ok) {
          ...
          setDbStatus("Sẵn sàng ghi dữ liệu.", "info");
        } else {
          setSettingsStatus(
            "Lưu thất bại: " + (response && response.error ? response.error : "unknown error"),
            "error"
          );
        }
      }
